...
export default async (req, res) => {
  if (req.method === 'POST') {
    try {
      ...
      // Your logic to handle the response and return as needed
      res.status(200).json({ success: true, response: chatResponse.data });
    } catch (error) {
      console.error(error);
      res.status(500).send(error.toString());
    }
  } else {
    // Handle any non-POST requests if necessary
